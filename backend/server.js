...
app.get("/api/product-usage", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: "Failed to fetch product usage data", details: err.message });
    }
});
...
app.get("/api/product-usage/timeseries", async (req, res) => {
    ...
    try {
        ...
        const topProducts = topProductsResult.rows.map(row => row.product_name);
        ...
        res.status(500).json({ error: "Failed to fetch time series data", details: err.message });
    }
});
...
app.get("/api/product-usage/summary", async (req, res) => {
    ...
    try {
        ...
        const summary = {
            ...
            avg_transaction_value: parseFloat(result.rows[0].avg_transaction_value)
        };
